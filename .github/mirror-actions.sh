...
TAG=$1
GITHUB_LOGIN="EotT123"
RELEASE_BODY="Details available at [EotT123/SubTools@${TAG}](https://github.com/EotT123/SubTools/releases/tag/${TAG})."
...
if [[ "$GITHUB_REPOSITORY" != "EotT123/SubTools" ]]; then
  echo "This mirror script is only meant to be run from EotT123/SubTools, not ${GITHUB_REPOSITORY}. Nothing to do here."
  exit 0
fi
...
git clone "https://EotT123:${GITHUB_SUPER_TOKEN}@github.com/EotT123/${ACTION_KEYBASE_NOTIFICATIONS_REPO}.git" /tmp/${ACTION_KEYBASE_NOTIFICATIONS_REPO}
...
git clone "https://EotT123:${GITHUB_SUPER_TOKEN}@github.com/EotT123/${ACTION_AUTOMATIC_RELEASES_REPO}.git" /tmp/${ACTION_AUTOMATIC_RELEASES_REPO}
...
git clone "https://EotT123:${GITHUB_SUPER_TOKEN}@github.com/EotT123/${ACTION_INJECT_SSM_SECRETS_REPO}.git" /tmp/${ACTION_INJECT_SSM_SECRETS_REPO}
