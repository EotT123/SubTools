...
public class JPodnapisiApi extends XmlRPC {
	...
	public List<PodnapisiSubtitleDescriptor> searchSubtitles(String filename, int year, int season,
		...
		String url =
				"http://www.podnapisi.net/sl/ppodnapisi/search?sK=" + URLEncoder.encode(filename, "UTF-8")
						+ "&sJ=" + PODNAPISI_LANGS.get(sublanguageid);
		if (year > 0)
			url = url + "&sY=" + year;
		if (season > 0) {
		...
		if (doc != null) {
			...
			for (int i = 0; i < nList.getLength(); i++) {
				if (nList.item(i).getNodeType() == Node.ELEMENT_NODE
						&& nList.item(i).getNodeName().equals("subtitle")) {
